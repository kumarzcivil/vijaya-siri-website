...
function formatQuickFixDuration(
  d: { value: number; unit: string } | undefined,
): string | null {
  if (!d) return null;
...
const HOW_STEPS = [
  {
    ...
    description: "Pick the service that matches your problem.",
  },
  {
    step: "02",
    title: "Pick a Convenient Time",
    description: "Choose a suitable time.",
  },
  {
    step: "03",
    title: "We'll Fix It",
    description: "Our expert arrives and gets it fixed.",
  },
];
...
export default function QuickFixPage() {
  ...
  const [brokenImageIds, setBrokenImageIds] = useState<ReadonlySet<string>>(
    new Set(),
  );
  ...
  return (
    <>
      <SEO
        title="QuickFix | Plumbing, Electrical & Home Repair Services"
        description="Explore available QuickFix services from Vijaya Siri, including plumbing, electrical work, and home maintenance services."
        canonical="/quick-fix"
      />
      ...
      <div className="qf-page">
                ...
                .map((cat) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {count > 0 && (
                        <span className="qf-cat-tile-badge">{count}</span>
                      )}
                    </button>
                  );
                })}
        ...
        <section id="qf-services" className="qf-services">
          <div className="section-container">
            <div className="qf-section-header qf-section-header--row">
              ...
              <span className="qf-services-count">
            ...
            <div className="qf-services-grid">
              {filteredServices.map((svc) => {
                ...
                const price = svc.pricing.enabled
                  ? formatINR(svc.pricing.price ?? 0)
                  : null;
                ...
                return (
                  <article key={svc.id} className="qf-service-card">
                    <div className="qf-service-img">
                      ...
                      {svc.featured && (
                        <span className="qf-service-flag">Popular</span>
                      )}
                    </div>
                    <div className="qf-service-body">
                      ...
                      <div className="qf-service-footer">
                        <span className="qf-service-meta">
                          {price && (
                            <span className="qf-service-price">{price}</span>
                          )}
                          {duration && (
                            <span className="qf-service-duration">
                              {duration}
                            </span>
                          )}
                        </span>
            ...
            {filteredServices.length === 0 && (
              <div className="qf-services-empty">
                <p>
                  No services match your search. Try a different term or
                  category.
                </p>
              </div>
            )}
              ...
              <ul className="qf-trust-strip">
                {TRUST_POINTS.map((point, i) => (
                  <li
                    ...
                    style={{
                      transitionDelay: trustVisible ? `${i * 80}ms` : undefined,
                    }}
                  >
        ...
        <section className="qf-how">
          <div className="section-container">
            ...
            <div
              ref={howRef}
              className={`qf-how-steps ${howVisible ? "qf-how-steps--visible" : ""}`}
            >
              <ol className="qf-how-steps-list">
                {HOW_STEPS.map((step, i) => (
                  <li
                    ...
                    style={{
                      transitionDelay: howVisible ? `${i * 100}ms` : undefined,
                    }}
                  >
