...
export default function QuoteFormPage() {
  ...
  const handleSubmit = useCallback(
    async (e: FormEvent) => {
      ...
      const step1Errors = computeErrors(1);
      const step2Errors = computeErrors(2);
  ...
  const whatsappMessage = useMemo(() => {
    const line = [
      ...
      `Project: ${selectedProjectTypeLabel}`,
      data.area ? `Area: ${data.area}` : '',
      data.budget ? `Budget: ${selectedBudgetLabel}` : '',
      data.requirements ? `Requirements: ${data.requirements}` : '',
    ];
    return line.filter(Boolean).join('\n');
  }, [refId, data.fullName, selectedLocationLabel, selectedProjectTypeLabel, data.area, data.budget, selectedBudgetLabel, data.requirements]);

  const whatsappUrl = useMemo(
    () => `https://wa.me/919008855088?text=${encodeURIComponent(whatsappMessage)}`,
    [whatsappMessage]
  );
      ...
      <div className="quote-form-page">
        <div className="quote-form-container">
          <div className="quote-success">
            ...
            <h1 className="quote-success-title">Your Quote Request Is Ready</h1>
            <p className="quote-success-text">
              Your quote request has been prepared using the details you provided. Tap the button below to send your request to Vijaya Siri on WhatsApp.
              {'\n\n'}
              Once we receive your request, our team will review your requirements and get in touch with you.
            </p>
        ...
        <form className="quote-form-body" onSubmit={handleSubmit} noValidate>
          ...
          <div className="quote-nav">
            ...
            {step < 3 ? (
              ...
              <button type="submit" className="quote-btn quote-btn--submit" disabled={isSubmitting}>
